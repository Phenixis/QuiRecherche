	...
	useEffect(() => {
		...
		if (globeEl.current && !globeEl.current.hasChildNodes()) {
			const globe = new Globe(globeEl.current)
				...
				.hexBinPointLat((d: object) => parseInt((d as PointData).lat))
				.hexBinPointLng((d: object) => parseInt((d as PointData).lng))
				.hexBinPointWeight((d: object) => (d as PointData).ids.length)
				.hexBinResolution(4) // Plus la résolution est basse, plus les hexagones sont grands
				.hexMargin(0.1)
			...
			if (pointClicked && publicationToFetchId) {
				setPublicationToFetchId(
					pointClicked.ids.length - 1 >=
						pointClicked.ids.indexOf(publicationToFetchId) && pointClicked.ids.indexOf(publicationToFetchId) <= 5
						? pointClicked.ids[
