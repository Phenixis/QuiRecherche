...
type Transaction = PgTransaction<
  ...
  ExtractTablesWithRelations<typeof import("@/lib/db/schema")>
>;
  ...
  try {
    ...
    for (const record of records) {
      ...
      for (const inproc of inproceedingsList) {
        ...
        const doi = eeText.includes("https://doi.org/")
          ? eeText.replace("https://doi.org/", "")
          : null;
    ...
    await db.transaction(async (tx) => {
      ...
      for (const university of universities) {
        universityIds[university.NOM] = (
          await createUniversity(university.NOM, tx)
        )[0].id;
      }
      ...
      for (const paper of papers) {
        paperIds[paper.doi] = (
          await createPaper(
            ...
            paper["TYPE_PUBLICATION.id"],
            parseInt(paper.year),
            paper.pages.includes("-")
              ? parseInt(paper.pages.split("-")[0])
              : undefined,
            paper.pages.includes("-")
              ? parseInt(paper.pages.split("-")[1])
              : undefined,
            paper.ee,
            tx
          )
      ...
      for (const contribution of contributions) {
        await createContributions(
          contribution["RESEARCHER.PID"],
          contribution.position,
          paperIds[contribution["ARTICLE.doi"]],
          tx
        );
      }
  ...
  const data = await db
    ...
    .where(
      or(
        ...
        like(
          sql`LOWER(CONCAT(${researcherTable.first_name}, ' ', ${researcherTable.last_name}))`,
          `%${name.toLowerCase()}%`
        )
      )
    )
...
export async function getAllInfosResearcher(pid: string, tx?: Transaction) {
  ...
  let researcher = await conn
    ...
    .from(researcherTable)
    .where(
      and(eq(researcherTable.pid, pid), isNull(researcherTable.deletedAt))
    );
  ...
  const universities = await conn
    ...
    .innerJoin(
      affiliationTable,
      eq(universityTable.id, affiliationTable.universityId)
    )
    .where(
      and(
...
export async function getUniversitiesByResearcherPid(
...
export async function getAffiliationByUniversityId(
  universityId: number,
  tx?: any
) {
  return await (tx ? tx : db)
    .select()
    .from(affiliationTable)
    .where(eq(affiliationTable.universityId, universityId));
}
...
export async function updateTypePublication(
  id: number,
  name?: string,
  tx?: Transaction
) {
...
export async function getPaperFromResearcherPid(
  researcherPid: string,
  tx?: Transaction
) {
  return await (tx ? tx : db)
